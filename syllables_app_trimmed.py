...
def get_syllables_per_second(audio_file):
    ...
    syllable_offsets = [item['start'] for item in offsets[0] if item['char'] in ['p', 't', 'k']]

    if not syllable_offsets:
        return 0

    first_syllable_time = syllable_offsets[0] / sample_rate
    last_syllable_time = syllable_offsets[-1] / sample_rate
    syllable_count = len(syllable_offsets)

    syllable_duration = last_syllable_time - first_syllable_time
    syllables_per_second = syllable_count / syllable_duration
